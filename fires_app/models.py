...
class FireModis(models.Model):
    date = models.DateTimeField(u'дата и время (UTC) получения данных о MODIS', null=True)
    cdate = models.DateField(u'дата получения данных о MODIS', null=True)
    ctime = models.TimeField(u'дата и время (UTC) получения данных о MODIS', null=True)
...
class FireViirs(models.Model):
    date = models.DateTimeField(u'дата и время (UTC) получения данных о VIIRS', null=True)
    cdate = models.DateField(u'дата получения данных о VIIRS', null=True)
    ctime = models.TimeField(u'время (UTC) получения данных о VIIRS', null=True)
    satellite = models.ForeignKey(Satellite)
    confidence = models.DecimalField(u'достоверность (0-100%)', max_digits=3, decimal_places=0)
    frp = models.DecimalField(u'мощность пожара', max_digits=5, decimal_places=1)
